...
# ./manage.py run_load_emd --file_path=/usr/data/robasev2-local/210126_emd.csv
# ./manage.py run_load_emd --single=true --juso="인천광역시 서구 가좌동"
class Command(BaseCommand):
    ...
    def make_json_of_emd(self):
        ...
        print(targets.count())
        with open('/usr/data/robasev2-local/230601_emd.json', 'w') as f:
            to_write_total = []
    ...
    def load(self):
        with open(self.options['file_path'], 'r') as f:
            ...
            for row in csvreader:
                if row[0] == '서울특별시' or row[0] == '경기도' or row[0] == '인천광역시':
                    idx += 1
                    self.do_job(row[4])
                # if idx > 2: break
