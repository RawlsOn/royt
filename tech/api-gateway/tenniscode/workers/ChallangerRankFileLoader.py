...
class ChallangerRankFileLoader(RankFileLoader):
    def process_player(self, row):
        ranking, name, club1, club2, _, _, _, _ = row
        player, created = tenniscode_models.Player.objects.get_or_create(
            name= name,
            main_club_str= club1,
            defaults= {
                '기준일': self.기준일
        ...
        # 신인부로 저장하기 위함
        player.player_level = '신인부(남자)'
        if not created: player.save()
        self.player = player
